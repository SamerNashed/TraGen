...
void Generator::setNumerical(bool numerical) { num = numerical; }
...
/*
 * Provides a report on the success or lack thereof when using a YAML file
 * to modify a Generator object.
 */
...
/*
 * Loads a YAML file and modifies the Generator object.
 */
...
bool Generator::Load(std::string file) {                                         
  ...
  if (mother["Planning_Space"]["Dimension"].as<double>() < 1) {                  
    parse_error = true;                                                       
    cout << "Planning must occur in at least 1 dimension." << endl;           
    return false;                                                             
  }                                                                           
  double dim = mother["Planning_Space"]["Dimension"].as<double>();                  
  
  if (mother["Planning_Space"]["Order"].as<double>() < 1) {                  
    parse_error = true;                                                       
    cout << "Polynomial must have order at least 1." << endl;           
    return false;                                                             
  }                                                                           
  double n = mother["Planning_Space"]["Order"].as<double>();                        
  
  if (mother["Planning_Space"]["Constraints_Per_Segment"].as<double>() < 1) {                  
    parse_error = true;                                                       
    cout << "Inequality constraints must have at least one enforcement per segment." << endl;           
    return false;                                                             
  }                                                                           
  double cps = mother["Planning_Space"]["Constraints_Per_Segment"].as<int>();    
  
  double tol = mother["Planning_Space"]["Order"].as<double>();                      
  double numer = mother["Planning_Space"]["Numerical"].as<double>();                
  double max_deriv = mother["Planning_Space"]["Maximum_Derivative"].as<double>();
...
//bounds[][][2] = deriv
//bounds[][][3] = dim
//bounds[][][4] = typerepmat(durations(bounds(bidx).seg).^tpow,[length(t), 1])
//bounds[][][5] = value
//bounds[][][6] = segment

//dimension values begin at 1. NOT 0.
void Generator::constructAandb(vector<MatrixXd>* powers, 
	vector<MatrixXd>* D, MatrixXd segs) {
  ...
  for (size_t i=0; i<bounds->size(); i++) {
    ...
    deriv = bounds[0][i][2];
    if (bounds[0][i][4] > 0) {//upper bound
      for (int j=0; j<cps; j++) {
        for (int k=0; k<(n+1); k++) {
	      Aineq(row, start_col + k) = (D[0][deriv](0,k)*pow(times[j],double(powers[0][deriv](0,k)))) /
              pow(segs(seg,0), double(n-k));
        }
...
int main(){
  ...
  */
  ...
  TraGEN->constructAandb(powers, LDList, segs);
